...
# --- 2. QUARANTINE TABLE ---
@dlt.table(name="gold_quarantined")
...
def apply_scd2_logic(target_base_name, target_final_name, source_view, keys, sequence_col, table_desc, source_name, expectations=None):
    ...
    dlt.apply_changes(target=target_base_name, source=source_view, keys=keys, sequence_by=F.col(sequence_col), stored_as_scd_type="2")

    @dlt.table(name=target_final_name, comment=table_desc)
    @dlt.expect_all_or_drop(expectations or {})
    def final_table():
...
# --- 7. MASTER GEOGRAPHY & FACT TABLES ---
@dlt.table(name="dim_geography_gold")
...
@dlt.table(
    ...
    comment="Fact table with a monotonically increasing Primary Key."
)
...
def fact_market_metrics_gold():
    ...
    # Generate the Primary Key using monotonically_increasing_id
